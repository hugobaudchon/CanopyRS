...
def iou_loss(inputs, targets, pred_ious, num_objects, use_l1_loss=False):
    ...
    """
    # Binarize predicted mask from probabilities
    prob_mask = inputs.sigmoid()
    pred_mask = (prob_mask > 0.5).flatten(1)   # [N, HW] bool
    gt_mask = (targets > 0.5).flatten(1)       # [N, HW] bool
    
    area_i = torch.sum(pred_mask & gt_mask, dim=-1).float()
    area_u = torch.sum(pred_mask | gt_mask, dim=-1).float()
    actual_ious = area_i / torch.clamp(area_u, min=1.0)   # [N]
    
    # Predicted IoUs are logits → map to [0,1]
