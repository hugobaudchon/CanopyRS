...
def timeseries_sam2(input_folder:str, output_folder:str, bbox_file:str):
    ann_frame_idx=0
    ...
    bboxes = bboxes[:10] # for testing, only take x bboxes for now
    bboxes.reset_index()
    ...
    #     fig, ax = plt.subplots(figsize=(6, 4))  # Use `fig` and `ax` for saving
    #     ax.set_title(f"frame {out_frame_idx}")
    #
    #     # Load and display the original frame
    ...
    #         show_mask(out_mask, ax, obj_id=out_obj_id)  # Ensure masks are added to `ax`
    #
    #     # Save the frame with the overlaid masks
    #     save_path = os.path.join(output_folder, f"frame_{out_frame_idx:04d}.png")
...
if __name__=="__main__":
    timeseries_sam2(
        ...
        '../montreal_forest_data/nice_cut/output',
        '../montreal_forest_data/nice_cut/0_formain_gr0p05_infer.gpkg'
    )