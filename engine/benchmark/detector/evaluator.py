...
def _get_size_category(area: float, area_ranges: dict) -> str:
    """Helper to determine size category from area."""
    if area_ranges['small'][0] <= area < area_ranges['small'][1]:
        return 'small'
...
class CocoEvaluator:
   ...
   # small_max_pixels = 3824
    ...
    def tile_level(self,
               ...
               compute_segmentation_quality: bool = True,
        ...
        """
        small_px = self.small_max_pixels
        medium_px = self.medium_max_pixels

        area_ranges = {
            'small': (0, small_px),        # < 32x32 pixels
            'medium': (small_px, medium_px),   # 32x32–96x96
            'large': (medium_px, 1e10)      # > 96x96
        }
        ...
        coco_evaluator.params.areaRng = [
            [0, 1e10],           # all
            [0, small_px],       # small
            [small_px, medium_px],  # medium
            [medium_px, 1e10],   # large
        ]
    ...
    def raster_level(self,
        ...
        metrics['F1_50'] = safe_f1(metrics['AP50'], metrics['AR50'])
        metrics['F1_75'] = safe_f1(metrics['AP75'], metrics['AR75'])
        metrics['F1_small'] = safe_f1(metrics['AP_small'], metrics['AR_small'])
        metrics['F1_medium'] = safe_f1(metrics['AP_medium'], metrics['AR_medium'])
        metrics['F1_large'] = safe_f1(metrics['AP_large'], metrics['AR_large'])
