...
from experiments.resolution.evaluate.get_wandb import wandb_runs_to_dataframe, extract_ground_resolution_regex
...
def evaluate_extent_and_architecture(wandb_project: str, architecture: str, extent: str, raw_root: str, models_root: str, output_folder, temp_folder):
    ...
    architecture_name = architecture.split('.')[0].split('/')[-1]
    output_folder = f"{output_folder}/{extent}/{architecture_name}"
    ...
    wandb_df['extent'] = wandb_df['valid_dataset_names'].apply(lambda x: tilerized_name_to_extent[x[0].split('/')[-1]])
    ...
    for fold, fold_inputs in inputs.items():
        ...
        for (tile_size, ground_resolution) in list(wandb_df[['augmentation_image_size', 'ground_resolution']].drop_duplicates().itertuples(index=False, name=None)):
            ...
            for dataset_name, raster_list in fold_inputs.items():
                for raster in raster_list:
                    ...
                    product_name, _, _, _ = CocoNameConvention.parse_name(coco_path)
    ...
    best_model_config.checkpoint_path = find_model_checkpoint_path(models_root, best_model_config.name)
    ...
    for product_name, tilerized_product_paths in tilerized_paths['valid'][(best_model_config.augmentation_image_size, best_model_config.ground_resolution)].items():
        tiles_path = tilerized_product_paths['tiles_path']

        io_config = InferIOConfig(
            input_imagery=None,
            tiles_path=tiles_path,
            output_folder=Path(output_folder) / 'best_model' / product_name,
        )
    ...
    for idx, model_row in wandb_df.iterrows():
        for product_name, tilerized_product_paths in tilerized_paths['test'][(model_row['augmentation_image_size'], model_row['ground_resolution'])].items():
            tiles_path = tilerized_product_paths['tiles_path']
            # Create a DetectorConfig for the current model.
            detector_config = DetectorConfig(**model_row.to_dict())
            detector_config.checkpoint_path = find_model_checkpoint_path(models_root, detector_config.name)
            ...
            pipeline = Pipeline(
                io_config=InferIOConfig(
                    input_imagery=None,
                    tiles_path=tiles_path,
                    output_folder=Path(output_folder) / 'test' / detector_config.name / product_name,
                ),
            ...
            all_tile_level_metrics.append(
                {
                    'model_name': detector_config.name,
